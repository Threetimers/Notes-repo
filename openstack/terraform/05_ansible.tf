...
resource "null_resource" "openvpn_server" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ANSIBLE_CONFIG=../ansible/ansible.cfg ansible-playbook -u debian -i /tmp/openvpn.ini --private-key ~/.ssh/info/id_rsa  ../ansible/openvpn_server.yml;
...
resource "null_resource" "create_new_vpn_client" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "[openvpn]" | tee -a /tmp/openvpn.ini;
      echo "openvpn ansible_host=${openstack_networking_floatingip_v2.floatip_1.address}" | tee -a /tmp/openvpn.ini;
      ANSIBLE_CONFIG=../ansible/ansible.cfg ansible-playbook -u debian -i /tmp/openvpn.ini --private-key ~/.ssh/info/id_rsa -e vpn_user_list=${each.value} ../ansible/openvpn_client.yml;
      rm -f /tmp/openvpn.ini;
    EOT
  }
